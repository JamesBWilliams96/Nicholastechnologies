...
const icons = [MessageIcon, CodeIcon, FileTextIcon, LifeBuoyIcon];

/** "The one-person advantage" — the model, stated and drawn. Dark tone. */
export function Studio({ locale, t }: { locale: Locale; t: Dictionary["studio"] }) {
  return (
        ...
        <div className="grid gap-8 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)] lg:items-end lg:gap-12">
          ...
          <Reveal delay={120} className="lg:justify-self-end">
            <div className="max-w-[46ch] lg:pb-1.5">
              <p className="text-lead text-muted">{fmt(t.body, { site: site.name })}</p>
              <Button href={localePath(locale, "#process")} variant="secondary" arrow className="mt-7">
                {t.cta}
              </Button>
            </div>
          </Reveal>
